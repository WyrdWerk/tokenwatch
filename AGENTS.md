...
- **SEO files must be deployed**: `sitemap.xml`, `robots.txt`, `favicon.svg`, and `og/og-image.svg` are committed and deployed with `public/`. Run `npm run seo` before deploy so the server-rendered table, sitemap, and robots are current. Cloudflare injects its own managed `robots.txt` rules (AI-crawler disallows) alongside the committed one — that's expected.
