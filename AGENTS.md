...
- **Tier 1 — Direct providers** (authoritative): DeepInfra, EmberCloud, Wafer, Synthetic, Lilac, SambaNova, HyperCharm, Sference, Neuralwatt, Merius, Aster Labs, SingularityAPI, RunInfra — fetched via their own `/v1/models` endpoints. SingularityAPI and RunInfra require `SINGULARITY_API_KEY` / `RUNINFRA_API_KEY` (skipped with a warning if unset). LLM Gateway (`LLMGATEWAY_API_KEY`) is also Tier 1 but **differential only**: it emits priced text rows for hosts TokenWatch does not already fetch (ByteDance, Runware, SCX, Canopywave, Iceberg, NanoGPT, …). Image-output SKUs and already-covered backends (OpenAI, Azure, Novita, Z.ai, …) are dropped.
...
- **Tier 3 — CSV/hardcoded/scraped**: Makora, Xiaomimimo (CSV), OpenCode Go (docs-page scrape: `parseOpenCodeGoDocs()` in `scripts/lib.mjs` scrapes the pricing table at `https://opencode.ai/docs/go/` — the `/zen/go/v1/models` catalog endpoint lists models but has no prices; context lengths stay manual via `OPENCODE_GO_CONTEXT`; legacy hardcoded array is the fallback), Umans (manually maintained `UMANS_MODELS` / `parseUmansHardcoded()` — not a live `/v1/models` fetch; status.umans.ai SSR is for performance data only), **Zro** (`scripts/fetch-zro.mjs` — official public page `https://zro.moonmath.ai/pricing`, parsed by `parseZroPricingHtml()` in `scripts/lib.mjs` from its Next.js React-Flight payload; ingested BEFORE OpenRouter so Zro's own tariff wins over any OpenRouter Zro backend. Daily cadence via a committed last-good snapshot `data/zro-pricing.json` with a 24h TTL — the 2-hourly refresh reuses it within 24h and on any fetch/parse/validation failure. Fail-closed validation: API panel required, unique ids, positive input/output, min 5 rows, ≤20% row-count drop vs last-good; zero/unpatched rows are dropped, never shown as free)
...
- **Consolidated FAQ**: `renderFaqPage()` in seo-pages generates `public/faq/index.html` (text + image + video + benchmark FAQ groups, owns the FAQPage JSON-LD). Calculator pages carry only a pointer section linking /faq/. `llms.txt` generated per refresh (buildLlmsTxt). Nav order: Text · Image · Video · Benchmarks · Providers · Methodology · API · FAQ. verify-seo: sitemap = providers + models + 9; FAQ/JSON-LD parity enforced on /faq/ only.
- **Canonical-model summary (text calculator)**: when the model search resolves to exactly one canonical id, a summary panel (`#modelSummary`, `renderModelSummary()` in `app.js`) replaces the flat table header area and shows: cheapest provider for the visitor's current mix, provider-offering input/output ranges, a CSS-bar provider-price distribution, cache-read coverage, and the full sortable offering table. Math is `shared/model-summary.mjs` (`canonicalSummary`/`rankOfferings`/`priceDistribution`), mirrored in `app.js` (`canonicalSummary`/`rankCanonicalOfferings`) and pinned by `test/model-summary.test.mjs`. Quant/SKU/`:batch` variants stay distinct rows; performance joins on `canonicalModelId(id) + '|' + provider` and stays `null` when absent. Labels are source-accurate: uptime is the 30-minute endpoint metric, latency/TTFT names its source — never a one-day uptime or a generic cache-hit-rate claim.
...
- **SEO**: `scripts/generate-seo.mjs` (build-time, `npm run seo`) server-renders the 25 cheapest models (by effective cost at the agentic mix 2.5/97/0.5) into `index.html` as a real HTML table — this is the crawlability fix for the client-side-rendered SPA. It also generates `sitemap.xml` (with `lastmod` from `pricing.json` `generated_at`) and `robots.txt`, the `/providers/` tree, and the `/models/<canonical>/` tree (100 pages at the ≥3-distinct-priced-providers floor, `:batch` canonicals excluded, quantized canonical ids kept distinct). **Idempotent**: checks for an existing `class="seo-models"` section and replaces it via regex — never re-inserts before `</main>`; `/providers/` and `/models/` are staged into a temp tree and atomically renamed. All 3 HTML pages carry keyword-rich titles/descriptions, canonical, og:image (1200×630), twitter:card=summary_large_image, and JSON-LD (WebSite + SoftwareApplication + FAQPage + BreadcrumbList). `_headers` sets `X-Robots-Tag: noindex` on `/api/*` and the raw JSON data files so they don't compete with HTML pages.
- **API**: Cloudflare Pages Functions at `functions/api/v1/` serve queryable endpoints: `/api/v1/models` (with filters: org, provider, min_context, min_output, min_intelligence, quantization, cache_read, cache_write, promo, zdr, sub, benchmarked, search, sort), `/api/v1/models/:id/providers` (mix-aware cost sort), `/api/v1/models/:id/history` (D1 price history), `/api/v1/stats` (org/zdr/sub/quantization breakdowns), `/api/v1/orgs`, `/api/v1/providers` (?zdr=true), `/api/v1/images`, `/api/v1/images/:id`, `/api/v1/videos`, `/api/v1/videos/:id`. CORS enabled. Invalid `%`-encoding returns 400; pagination clamps limit∈[1,500], offset≥0.
...
- DeepInfra / HyperCharm / Aster Labs / Makora / Xiaomimimo / OpenCode Go / SingularityAPI / RunInfra: $/M (passthrough)
- Wafer: cents/M → ÷100
...
- **Other direct providers** (Wafer, Lilac): ID-based regex fallback for embeddings/TTS (no structured metadata available).
...
| `functions/api/v1/[[route]].js` | Cloudflare Pages Functions API — imports `canonicalId` from `shared/normalize.mjs`. /models (with filters: org, provider, min_context, min_output, quantization, cache_read, cache_write, promo, zdr, sub, search, sort), /models/:id/providers (mix-aware cost sort), /models/:id/history (D1 price history), /stats (org/zdr/sub/quantization breakdowns), /orgs, /providers (?zdr=true), /images, /images/:id, /videos, /videos/:id, CORS |
| `shared/price-history.mjs` | Pure price-history helpers (offering identity, raw-row normalization, read-time blending, day capping, `days`/`mix` parsing, retention windows). Worker-safe, no `node:` imports. |
| `scripts/snapshot-prices.mjs` | Daily snapshot writer — reads `public/pricing.json`, claims the UTC day in `price_snapshot_day` and writes one row per offering for it, plus an unguarded 90-day retention delete, all in one transactional D1 batch. First successful refresh after 00:00 UTC wins the day: a later same-day run writes nothing. `--force` is an explicit operator repair that replaces the day's rows and refreshes its claim. `--local` only; remote writes are deliberately unwired. |
| `migrations/0001_price_history.sql` | D1 schema for `price_snapshot`: raw USD/M components, unique `(offering_key, utc_day)` row key, model/day and day indexes. |
| `migrations/0002_snapshot_day_claim.sql` | D1 schema for `price_snapshot_day`: the day-level first-success claim (`claimed_at`, `offering_count`, `source_generated_at`, `claim_token`) that makes one catalog own a day. See `migrations/README.md`. |
| `public/price-sparkline.js` | Dependency-free SVG sparkline with explicit loading/empty/error/single/flat/provider-switch states. |
| `public/model-history.js` | Wires the sparkline to `/api/v1/models/:id/history`, following the visitor's saved `tw-mix` (raw CSV, as `public/app.js` writes it). Auto-mounts `[data-price-history]`. |
| `public/docs/price-history-states/index.html` | Visual reference for every sparkline state (noindex). |
...
| `.github/workflows/refresh-pricing.yml` | Main pipeline: `test` (push, runs `node --test`), `refresh` (every 2h cron: test→fetch all pipelines→fetch-performance→commit 4 JSONs→generate-seo→SEO integrity guard→bust-cache→minify-json→deploy→/h/* smoke; post-commit steps gated on `changed`/`inputs.force`), `deploy` (push: generate-seo→guard→bust-cache→minify→deploy→smoke). Performance data (latency/throughput) sourced from OR (primary, requires key) + Lilac/Umans (direct). SEO artifacts regenerate every deploy (committed index.html may be stale; `npm run seo` regenerates from committed pricing.json). Cache-busted HTML + `/h/` copies are never committed. |
| `data/manual-pricing.csv` | Static pricing for CSV-sourced providers (Makora, Xiaomimimo) |
| `data/zro-pricing.json` | Committed last-good Zro API-pricing snapshot (`source_url`, `fetched_at`, per-model current/original prices + promotion metadata). Refreshed at most daily via a 24h TTL; also the fallback when the live page fails. Committed by `refresh-pricing.yml`. |
| `scripts/fetch-zro.mjs` | Zro Tier-3 ingestion: fetch → parse (`parseZroPricingHtml`) → fail-closed validate (`validateZroSnapshot`) → snapshot TTL reuse/fallback. Exposes `getZroCatalogRows()` which always returns ordinary catalog rows (provider `zro`) or last-good rows — never a partial slice, never zero-priced models as free. |
| `shared/model-summary.mjs` | Pure canonical-model summary math (`canonicalSummary`, `rankOfferings`, `priceDistribution`) shared by the calculator and tests. Worker-safe (no `node:` imports). Mirrored in `app.js` and pinned by `test/model-summary.test.mjs`. |
...
npm run snapshot        # Write today's price snapshot into LOCAL D1 (needs a prior `wrangler d1 migrations apply --local`)
```
...
- `GET /api/v1/models/:canonicalId/history` — daily cheapest-provider price history from D1 (`PRICE_HISTORY` binding). Optional `?days=1..90` (default 90) and `?mix=inputPct,cacheReadPct,outputPct` (default 2.5,97,0.5). Raw snapshots are blended **at read time**, so the same rows serve any workload. Returns ≤90 chronologically ordered points; a day with no snapshot is absent, never `$0`. Malformed/out-of-range params are 400; an unknown model is 404; a missing binding is 503. See [docs/adr/0011-price-history-snapshots.md](docs/adr/0011-price-history-snapshots.md) and `migrations/README.md`.
...
5. **Historical price tracking**: Local-only daily snapshots shipped (D1 `PRICE_HISTORY`, `scripts/snapshot-prices.mjs`, `/api/v1/models/:canonicalId/history`, `public/price-sparkline.js`). Production rollout — real D1 database, remote binding, remote migration, CI writer step — is not approved yet. Price-drop alerts still unbuilt.
