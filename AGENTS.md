...
- **Tier 1 — Direct providers** (authoritative): DeepInfra, Crof, EmberCloud, Wafer, Synthetic, Lilac, SambaNova, Hyper, Sference, Neuralwatt — fetched via their own `/v1/models` endpoints
...
- **ZDR (Zero Data Retention)**: Two-stage tagging in `main()`:
  ...
  - `MANUAL_PROVIDER_META` includes `retains_prompts`, `may_train`, `retention_days` for 8 manual providers (crof, ember, hyper, lilac, makora, synthetic, opencode, xiaomimimo) based on privacy policy review. Neuralwatt has URL-only metadata (privacy/terms/status) — ZDR/retention status not determined (no public evidence).
