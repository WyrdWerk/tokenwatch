...
|---|---|---|
| Text `/` | all 19 below | `org`, `provider`, `model`, `input`, `output`, `cache_read`, `context`, `speed`, `blended`, `cost` |
| Image `/image` | `get_view`, `get_catalog_info`, `set_sort` | `org`, `model`, `cost_per_unit`, `cost` |
| Video `/video` | `get_view`, `get_catalog_info`, `set_sort` | `org`, `model`, `resolution`, `audio`, `cost_per_second`, `cost` |
