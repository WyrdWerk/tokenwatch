...
async function main() {
  ...
  // 3. Resolve the model-creator org for the group (logic in shared/benchmark-org.mjs).
  ...
  // variants (fast/turbo) inherit the same creator as the base id. The four
  // layers — clean org, provider-slug blocklist, family inheritance, leading-token
  // map — live in buildOrgIndex() so they can be unit-tested hermetically.
  const providerSlugs = pricing.providers.map((p) => String(p.key || '').toLowerCase());
  const orgByCid = buildOrgIndex(
    new Map([...byCanonical].map(([cid, info]) => [cid, info.offerings])),
    providerSlugs,
  );
